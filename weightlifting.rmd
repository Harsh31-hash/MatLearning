...
This is a work i have produced for my data science specialization sub course and the analysis is about working on different technique. this is the compilation of the work that i have done for the past two days and i hope its correct. this should be good enough. please dont make me suffer anymore. your software has been  real piece of shit.
...
for(c in c(col_a,col_b,col_c,col_d,col_e)){
  sab_bad <- union(sab_bad, c)
}
# Pappa rappi paana pao pao pao
# Pappa rappi paana pao pao pao
# Pappa rappi paana pao pao pao
# Pappa rappi paana pao pao pao
# Pappa rappi paana pao pao pao
# Pappa rappi paana pao pao pao
vwx2 <- vwx %>% select(sab_bad, classe)
data.frame("arm" = sum(grepl("_arm",sab_bad)), 
           "forearm" = sum(grepl("_forearm",sab_bad)),
           "belt" = sum(grepl("_belt",sab_bad)),
           "dumbbell" = sum(grepl("_dumbbell",sab_bad)))
```
...
```
```{r}
abcF <- abc %>% select(sab_bad,classe)
ghiF <- ghi %>% select(sab_bad,classe)
...
# Pappa rappi paana pao pao pao
# Pappa rappi paana pao pao pao
def2 <- def %>% select(sab_bad,problem_id)
mazze <- def2 %>% select(sab_bad)
  
result <- data.frame("problem_id" = def$problem_id,
                     "PREDICTION_RF" = predict(modelRF,mazze),
                     "PREDICTION_GBM" = predict(modelGBM,mazze),
                     "PREDICTION_SVM" = predict(modelSVM,mazze))
result
```
